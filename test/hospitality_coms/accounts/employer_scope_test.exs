defmodule HospitalityComs.Accounts.EmployerScopeTest do
  @moduledoc """
  The employer scope refuses to be built out of something that is not a venue
  id.

  `venue_id` is not an inert field. It is written into the transaction as
  ...
  U9 reads the employer zone through or of a row-level security policy, with a
  Postgres message about a cast.
  """
  ...
  describe "for_employer/2" do
    test "builds a scope from a UUID and the unit of work's instant" do
      ...
      assert %EmployerScope{venue_id: ^venue_id, now: @now} =
               EmployerScope.for_employer(venue_id, @now)
    end

    test "normalises an uppercase UUID to the form Postgres hands back" do
      venue_id = Ecto.UUID.generate()

      assert EmployerScope.for_employer(String.upcase(venue_id), @now).venue_id ==
               venue_id
    end

    test "refuses the empty string" do
      assert_raise ArgumentError, ~r/is not a venue id/, fn ->
