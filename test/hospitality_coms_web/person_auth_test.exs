defmodule HospitalityComsWeb.PersonAuthTest do
  ...
  @remember_me_cookie "_hospitality_coms_web_person_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, HospitalityComsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_person/3" do
    ...
    test "redirects to settings when person is already logged in", %{conn: conn, person: person} do
      conn =
        conn
        |> assign(:current_scope, Scope.for_person(person))
        |> PersonAuth.log_in_person(person)
    ...
    test "writes a cookie if remember_me was set in previous session", %{conn: conn, person: person} do
      conn = conn |> fetch_cookies() |> PersonAuth.log_in_person(person, %{"remember_me" => "true"})
      assert get_session(conn, :person_token) == conn.cookies[@remember_me_cookie]
      assert get_session(conn, :person_remember_me) == true
  ...
  describe "logout_person/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      HospitalityComsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_scope_for_person/2" do
    test "authenticates person from session", %{conn: conn, person: person} do
      ...
      conn =
        conn |> put_session(:person_token, person_token) |> PersonAuth.fetch_current_scope_for_person([])
    ...
    test "reissues a new token after a few days and refreshes cookie", %{conn: conn, person: person} do
      logged_in_conn =
        conn |> fetch_cookies() |> PersonAuth.log_in_person(person, %{"remember_me" => "true"})
      ...
      {person, _} = Accounts.get_person_by_session_token(token)
  ...
  describe "on_mount :mount_current_scope" do
    ...
    test "assigns current_scope based on a valid person_token", %{conn: conn, person: person} do
      person_token = Accounts.generate_person_session_token(person)
      session = conn |> put_session(:person_token, person_token) |> get_session()

      {:cont, updated_socket} =
        PersonAuth.on_mount(:mount_current_scope, %{}, session, %LiveView.Socket{})

      assert updated_socket.assigns.current_scope.person.id == person.id
    end

    test "assigns nil to current_scope assign if there isn't a valid person_token", %{conn: conn} do
    ...
    test "assigns nil to current_scope assign if there isn't a person_token", %{conn: conn} do
  ...
  describe "on_mount :require_authenticated" do
    test "authenticates current_scope based on a valid person_token", %{conn: conn, person: person} do
      person_token = Accounts.generate_person_session_token(person)
      session = conn |> put_session(:person_token, person_token) |> get_session()

      {:cont, updated_socket} =
        PersonAuth.on_mount(:require_authenticated, %{}, session, %LiveView.Socket{})
    ...
    test "redirects to login page if there isn't a valid person_token", %{conn: conn} do
      ...
      {:halt, updated_socket} = PersonAuth.on_mount(:require_authenticated, %{}, session, socket)
      assert updated_socket.assigns.current_scope == nil
    end
  ...
  describe "on_mount :require_sudo_mode" do
    test "allows people that have authenticated in the last 10 minutes", %{conn: conn, person: person} do
      person_token = Accounts.generate_person_session_token(person)
      session = conn |> put_session(:person_token, person_token) |> get_session()
      ...
      assert {:cont, _updated_socket} =
               PersonAuth.on_mount(:require_sudo_mode, %{}, session, socket)
    end
    ...
    test "redirects when authentication is too old", %{conn: conn, person: person} do
      ...
      person_token = Accounts.generate_person_session_token(person)
      {person, token_inserted_at} = Accounts.get_person_by_session_token(person_token)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        HospitalityComsWeb.Endpoint.subscribe("people_sessions:#{Base.url_encode64(token)}")
      end

      PersonAuth.disconnect_sessions(tokens)

      assert_receive %Phoenix.Socket.Broadcast{
        event: "disconnect",
        topic: "people_sessions:dG9rZW4x"
      }

      assert_receive %Phoenix.Socket.Broadcast{
        event: "disconnect",
        topic: "people_sessions:dG9rZW4y"
      }
    end
  end
end
